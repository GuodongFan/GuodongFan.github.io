...
`To make the SSTN+SSPE module adapt to the imperfect human proposals generated by the human detector.`

The distribution of the relative offset between the detected bounding box and the ground truth bounding box varies across different poses. In another words, there exists a distribution $P(\delta B\|P)$, where $\delta B$ is the offset, and $P$ is the ground truth pose of a person.
